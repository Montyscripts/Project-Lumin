...
:: 0. Initialize workspace directories and default config
...
:: 3. Check frontend packages and ensure fast production build
...
if not exist "%PROJ_DIR%\dist\index.html" (
    echo [STATUS] Building optimized production frontend bundle for instant loading...
    call "!NPM_CMD!" run build
)

:: 4. Check Ollama daemon
set "OLLAMA_EXE="
...
if defined OLLAMA_EXE (
    ...
    if errorlevel 1 (
        echo [STATUS] Starting local Ollama background service...
        powershell -NoProfile -Command "Start-Process -FilePath '!OLLAMA_EXE!' -ArgumentList 'serve' -WindowStyle Hidden" >nul 2>&1
    )
)

:: 5. Safe Port 3000 & Process Management (only terminate prior LUMIN instances)
