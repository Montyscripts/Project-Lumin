...
vec3 calc( vec3 pos ) {
  ...
  // Apply audio vertex displacement
  float actualDisplacement = displacement;
  vec3 audio_disp = dir * (actualDisplacement * morphingEnabled);
...
void main() {
  ...
  transformedNormal = normalMatrix * normalize(np);
